...
\begin{itemize}%
\item In this unit, we want to get a rough feeling about what \emph{optimization} is.%
...
\begin{itemize}%
\item Many questions in the real world are \emph{optimization problems}\only<2-8>{, e.g.,%
\begin{itemize}%
%
\item Find the \emph{shortest} tour for a salesman to visit a certain set of cities\only<-2>{ in China and return to Hefei!}%
%
...
%
\item<5-> \only<-5>{I want to build a large factory with $n$ workshops. I know the flow of material between each two workshops and now need to choose the locations of the workshops such that the overall running cost incurred by material transportation is \emph{minimized}.}\only<6->{Assign workshops to locations}%
%
\item<6-> \only<-7>{Which setting of $x_1$, $x_2$, $x_3$, and $x_4$ can make $(x_1\lor\lnot x_2 \lor x_3)\land(\lnot x_2\lor\lnot x_3 \lor x_4) \land (\lnot x_1\lor\lnot x_3 \lor \lnot x_4)$ become true\only<6>{?}\only<7->{ (or, at least, as \emph{many} of its terms as possible)?}}\only<8->{Satisfy Boolean formula}%
%
...
\item<5-> These are some of the ingredients.\uncover<6->{ They do not make the production \emph{intelligent} yet.\uncover<7->{ They are technological enablers.}}%
...
\uncover<9->{~\dots\alert{can make systems \emph{intelligent}}}%
\end{itemize}%
\end{frame}%
%
%
...
\begin{itemize}%
\item In optimization, there exist \emph{exact} and \emph{heuristic} algorithms.%
%
...
\only<-9>{\item<8-> But we can find just \emph{some} tour very quickly.}%
...
\item<13-> \alert{Optimization often means to make a trade-off between solution quality and runtime\only<14->{ and \emph{development time (the time from the definition of the problem until we have a software for producing solutions)}}.}%
\end{itemize}%
}%
\end{itemize}%
%
